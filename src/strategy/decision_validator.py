"""
Decision Validator
Validates the legality and safety of LLM decisions
"""
...
class DecisionValidator:
    ...
    def validate(self, decision: Dict) -> Tuple[bool, List[str]]:
        ...
        # 1. Required field check (needed for all action types)
        required_fields = ['symbol', 'action', 'reasoning']
        for field in required_fields:
            if field not in decision:
                errors.append(f"Missing required field: {field}")
        
        # If basic fields are missing, return immediately
        if errors:
            return False, errors
        
        # 2. Action legality check (unified action protocol)
        ...
        # 5. Additional checks for open position operations
        if decision['action'] in OPEN_ACTIONS:
            # 5.1 Required fields for opening positions
            ...
            for field in open_required:
                if field not in decision or decision[field] is None:
                    errors.append(f"Open position operation missing required field: {field}")
            
            # If open position fields are missing, skip subsequent checks
            if any('Open position operation missing required field' in e for e in errors):
                return False, errors
            ...
            # 5.3 Position size check (if position_size_pct exists)
            ...
            for field in numeric_fields:
                if field in decision:
                    ...
                    if isinstance(value, str):
                        errors.append(f"{field} cannot be a string (may contain formula): {value}")
    ...
    def _validate_format(self, decision: Dict) -> List[str]:
        ...
        Check rules:
        1. Range symbol ~ is forbidden
        2. Thousands separator , is forbidden
        ...
        for key, value in decision.items():
            if isinstance(value, str):
                ...
                if re.match(r'^\d{1,3}(,\d{3})+(\.\d+)?$', value):
                    errors.append(f"Field {key} contains forbidden thousands separator ',': {value}")
    ...
    def validate_stop_loss_direction(self, decision: Dict) -> bool:
        ...
        # Only check open position operations
        ...
        # If price info is missing, cannot validate
    ...
    def validate_risk_reward_ratio(self, decision: Dict) -> bool:
        ...
        if ratio is None:
            return True  # Do not block if cannot calculate
    ...
    def calculate_risk_reward_ratio(self, decision: Dict) -> Optional[float]:
        ...
        Returns:
            Risk-reward ratio, or None if cannot calculate
        """
        action = normalize_action(
            decision.get('action'),
            position_side=decision.get('position_side')
        ...
        # Only calculate for open position operations
        if action not in OPEN_ACTIONS:
            return None
        ...
        # If price info is missing, cannot calculate
