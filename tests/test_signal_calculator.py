...
def test_rsi_no_division_by_zero():
    """Test RSI calculation does not divide by zero"""
    calc = BacktestSignalCalculator()
    
    # Create data with all zero changes (causes loss=0)
    data = pd.Series([100.0] * 50)
    
    # Should not raise exception
    rsi = calc.calculate_rsi(data)
    
    # RSI should be a valid value (not NaN or Inf)
